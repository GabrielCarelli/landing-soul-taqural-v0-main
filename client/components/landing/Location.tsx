  ...
  const PointItem = ({ title, desc }: { title: string; desc: string }) => (
    <div className="flex items-start gap-2 shrink min-w-0 basis-[clamp(160px,22vw,260px)]">
      ...
      <div className="min-w-0">
        <h3 className="text-soul-dark text-sm lg:text-base xl:text-lg font-bold leading-tight">{title}</h3>
        <p className="text-soul-dark/80 text-xs lg:text-sm leading-snug break-words">{desc}</p>
      </div>
    </div>
  );
